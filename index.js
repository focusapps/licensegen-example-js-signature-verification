...
async function main() {
  ...
  const uri = `/v1/accounts/${LICENSEGEN_ACCOUNT_ID}/${route.replace(/^\//, '')}`
  const res = await fetch(`https://licensegen-api.focusapps.app/${uri.replace(/^\//, '')}`, {
    method: 'GET',
    headers: {
  ...
  try {
    ...
    const verifyKey = crypto.createPublicKey({
      key: encodeHexKeyToDerFormat(LICENSEGEN_VERIFY_KEY),
      format: 'der',
      type: 'spki',
    })
